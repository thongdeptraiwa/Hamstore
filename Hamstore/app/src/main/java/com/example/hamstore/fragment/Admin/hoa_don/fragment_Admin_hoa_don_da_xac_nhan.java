...
public class fragment_Admin_hoa_don_da_xac_nhan extends Fragment {
    Context c;
    ...
    public void onStart() {
        ...
        FirebaseRecyclerAdapter adapter = new FirebaseRecyclerAdapter<Hoa_Don, ViewHolder>(options) {
            ...
            protected void onBindViewHolder(ViewHolder holder, int position, Hoa_Don model) {
                ...
                //đã thanh toán
                if(model.getTrang_thai().equals("Đã xác nhận")) {
                    //hien thi
    ...
    private void dialog_xanh(Hoa_Don hoaDon){
        ...
        //đổi text btn_xanh
        btn_xanh.setText("Đang vận chuyển");
        ...
        btn_xanh.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                data.child(hoaDon.getId()).child("trang_thai").setValue("Đang vận chuyển");
                dialog.dismiss();
            }
        });
