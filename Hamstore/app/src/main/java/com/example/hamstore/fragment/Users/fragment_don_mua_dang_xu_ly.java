...
public class fragment_don_mua_dang_xu_ly extends Fragment {
    Context c;
        ...
        FirebaseRecyclerAdapter adapter = new FirebaseRecyclerAdapter<Hoa_Don, ViewHolder>(options) {
            ...
            protected void onBindViewHolder(ViewHolder holder, int position, Hoa_Don model) {
                ...
                if(model.getId_user().equals(trangChu.tai_khoan) && (model.getTrang_thai().equals("Chờ xác nhận")
                    ...
                    if(model.getTrang_thai().equals("Đã xác nhận") || model.getTrang_thai().equals("Đang vận chuyển")){
                        //nhấn item hiện dialog chi tiết
                        holder.itemView.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
    ...
    private void dialog_huy_bo(Hoa_Don hoaDon){
